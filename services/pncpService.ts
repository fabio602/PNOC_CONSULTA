...
const EMPRESA_AQUI_KEY = '30f0e2d7f280dffdb608f77991475b3f14ace1c9';
...
export const fetchEmpresaInfo = async (cnpj: string): Promise<EmpresaInfo> => {
  const safeCnpj = cleanCnpj(cnpj);
  const url = `https://www.empresaqui.com.br/api/${EMPRESA_AQUI_KEY}/${safeCnpj}`;
  const finalUrl = `${PROXY_URL}${encodeURIComponent(url)}`;
  
  const response = await fetch(finalUrl);
  if (!response.ok) throw new Error("Falha ao consultar EmpresaAqui.");
  
  const result = await response.json();
  if (!result || (Array.isArray(result) && result.length === 0)) {
    throw new Error("Empresa não encontrada na base EmpresaAqui.");
  }
